			...
			<tr>
				...
				<td>
					<div style="text-align: right; margin: 5px 10px 5px 0px">
						...
						&nbsp;&nbsp;&nbsp;&nbsp;&gt;&nbsp;&nbsp;&nbsp; <a
							href="${pageContext.request.contextPath }/client/myAccount.jsp">&nbsp;我的帐户</a>
						&nbsp;&nbsp;&nbsp;&nbsp;&gt;&nbsp;&nbsp;&nbsp;&nbsp; 订单查询
					</div>
							...
							<td style="padding: 20px"><p>
								...
								<table width="100%" border="0" cellspacing="0" class="tableopen">
									<tr>
										...
										<td bgcolor="#A3D7E6" class="tableopentd01">图书</td>
										<td bgcolor="#A3CCE6" class="tableopentd01">订单时间</td>
										<td bgcolor="#A3B6E6" class="tableopentd01">状态</td>
										<td bgcolor="#A3c8E6" class="tableopentd01">归还时间</td>
										<td bgcolor="#A3E2E6" class="tableopentd01">操作</td>
									</tr>
									<c:forEach items="${orders}" var="order">
										<tr>
											<td class="tableopentd02">${order.id}</td>
											<td class="tableopentd02">${order.book_name }</td>
											<td class="tableopentd02">${order.time}</td>
											<td class="tableopentd02">${order.flag==false?"未成功":"已成功"}</td>
											<td class="tableopentd02">${order.return_time}</td>
											<td class="tableopentd03"><a
												href="${pageContext.request.contextPath}/FindOrderByIdServlet?id=${order.id}">查看</a>&nbsp;&nbsp;
												<c:if test="${order.flag==false }">
													<a
														href="${pageContext.request.contextPath}/DeleteOrderByIdServlet?id=${order.id}"
														onclick="javascript:return o_del()">刪除</a>
												</c:if> <c:if test="${order.flag==true }">
													<a
														href="${pageContext.request.contextPath}/DeleteOrderByIdServlet?id=${order.id}&type=client"
														onclick="javascript:return o_del()">刪除</a>
												</c:if></td>
